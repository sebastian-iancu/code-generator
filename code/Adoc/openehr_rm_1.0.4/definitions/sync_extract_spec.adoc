...
h|*1..1*
|*includes_versions*: `xref:BASE:foundation_types:primitive_types_package.adoc#_boolean_class[Boolean]`
a|True if the Versions from the Contribution are included; False if just the Contribution and its Audit are included.

h|*0..1*
|*contribution_list*: `xref:BASE:foundation_types:structures_package.adoc#_list_class[List]<xref:BASE:base_types:identification_package.adoc#_hier_object_id_class[HIER_OBJECT_ID]>`
...
h|*0..1*
|*all_contributions*: `xref:BASE:foundation_types:primitive_types_package.adoc#_boolean_class[Boolean]`
