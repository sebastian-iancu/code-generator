...
h|*1..1*
|*items*: `xref:BASE:foundation_types:structures_package.adoc#_list_class[List]<xref:RM:common:generic_package.adoc#_revision_history_item_class[REVISION_HISTORY_ITEM]>`
...
h|*1..1*
|*most_recent_version* (): `xref:BASE:foundation_types:primitive_types_package.adoc#_string_class[String]` +
 +
__Post__: `Result.is_equal (items.last.version_id.value)`
a|The version id of the most recent item, as a String.

h|*1..1*
|*most_recent_version_time_committed* (): `xref:BASE:foundation_types:primitive_types_package.adoc#_string_class[String]` +
 +
