...
h|*1..1*
|*identities*: `xref:BASE:foundation_types:structures_package.adoc#_list_class[List]<xref:RM:demographic_package.adoc#_party_identity_class[PARTY_IDENTITY]>`
a|Identities used by the party to identify itself, such as legal name, stage names, aliases, nicknames and so on.

h|*0..1*
|*contacts*: `xref:BASE:foundation_types:structures_package.adoc#_list_class[List]<xref:RM:demographic_package.adoc#_contact_class[CONTACT]>`
...
h|*0..1*
|*reverse_relationships*: `xref:BASE:foundation_types:structures_package.adoc#_list_class[List]<xref:BASE:base_types:identification_package.adoc#_locatable_ref_class[LOCATABLE_REF]>`
a|References to relationships in which this Party takes part as target.

h|*0..1*
|*relationships*: `xref:BASE:foundation_types:structures_package.adoc#_list_class[List]<xref:RM:demographic_package.adoc#_party_relationship_class[PARTY_RELATIONSHIP]>`
