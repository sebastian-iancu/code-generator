...
h|*1..1*
|*start_time*: `xref:RM:data_types:quantity_package.adoc#_dv_date_time_class[DV_DATE_TIME]`
...
h|*0..1*
|*end_time*: `xref:RM:data_types:quantity_package.adoc#_dv_date_time_class[DV_DATE_TIME]`
a|Optional end time of the clinical session.

h|*0..1*
|*location*: `xref:BASE:foundation_types:primitive_types_package.adoc#_string_class[String]`
a|The actual location where the session occurred, e.g. 'microbiology lab 2', 'home', 'ward A3'  and so on.

h|*1..1*
|*setting*: `xref:RM:data_types:text_package.adoc#_dv_coded_text_class[DV_CODED_TEXT]`
a|The setting in which the clinical session took place. Coded using the openEHR Terminology,  setting  group.

h|*0..1*
|*other_context*: `xref:RM:data_structures:item_structure_package.adoc#_item_structure_class[ITEM_STRUCTURE]`
a|Other optional context which will be archetyped.

h|*0..1*
|*health_care_facility*: `xref:RM:common:generic_package.adoc#_party_identified_class[PARTY_IDENTIFIED]`
...
h|*0..1*
|*participations*: `xref:BASE:foundation_types:structures_package.adoc#_list_class[List]<xref:RM:common:generic_package.adoc#_participation_class[PARTICIPATION]>`
