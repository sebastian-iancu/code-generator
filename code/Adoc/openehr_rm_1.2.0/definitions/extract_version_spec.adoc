...
h|*1..1*
|*include_all_versions*: `xref:BASE:foundation_types:primitive_types_package.adoc#_boolean_class[Boolean]`
a|True if all versions of each item in the Extract are included.

h|*0..1*
|*commit_time_interval*: `xref:RM:data_types:quantity_package.adoc#_dv_interval_class[DV_INTERVAL]<xref:RM:data_types:quantity_package.adoc#_dv_date_time_class[DV_DATE_TIME]>`
...
h|*1..1*
|*include_revision_history*: `xref:BASE:foundation_types:primitive_types_package.adoc#_boolean_class[Boolean]`
a|True if revision histories of the items in the Extract are included. If included, it is always the full revision history.

h|*1..1*
|*include_data*: `xref:BASE:foundation_types:primitive_types_package.adoc#_boolean_class[Boolean]`
