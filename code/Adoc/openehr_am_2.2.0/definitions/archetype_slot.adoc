[cols="^1,3,5"]
|===
h|*Class*
2+^h|*ARCHETYPE_SLOT*

h|*Description*
2+a|Constraint describing a  slot' where another archetype can occur.

h|*Inherit*
2+|`xref:AM:aom2:constraint_model_package.adoc#_c_object_class[C_OBJECT]`

h|*Attributes*
^h|*Signature*
^h|*Meaning*

h|*0..1*
|*includes*: `xref:BASE:foundation_types:structures_package.adoc#_list_class[List]<link:/classes/ASSERTION[ASSERTION] >`
a|List of constraints defining other archetypes that could be included at this point.

h|*0..1*
|*excludes*: `xref:BASE:foundation_types:structures_package.adoc#_list_class[List]<link:/classes/ASSERTION[ASSERTION] >`
a|List of constraints defining other archetypes that cannot be included at this point.

h|*1..1*
|*is_closed*: `xref:BASE:foundation_types:primitive_types_package.adoc#_boolean_class[Boolean]`
a|True if this slot specification in this artefact is closed to further filling either in further specialisations or at runtime. Default value False, i.e. unless explicitly set, a slot remains open.

h|*Functions*
^h|*Signature*
^h|*Meaning*

h|*1..1*
|*any_allowed* (): `xref:BASE:foundation_types:primitive_types_package.adoc#_boolean_class[Boolean]`
a|True if no constraints stated, and slot is not closed.
|===
